...
resource "azurerm_container_group" "aci-azdevops" {
    name = "aci-agent"
    location = "${azurerm_resource_group.aci-rg.location}"
    resource_group_name = "${azurerm_resource_group.aci-rg.location}"
    ip_address_type = "public"
    os_type = "linux"

    container {
        name = "azdevops-agent"
        image = "jacklwade/azdevops-infra-agent"
        cpu = "0.5"
        memory = "1.5"
        port = "80"
